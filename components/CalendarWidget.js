    ...
    return (
        <View style={[styles.widget, styles.darkWidget]}>
            ...
            <View style={styles.calendarGrid}>
                {days.map((day, index) => (
                    <TouchableOpacity key={index} style={styles.calendarCell} onPress={() => onDateSelect(day.dateKey)}>
                        ...
                        {day.eventCount > 0 &&
                            <View style={styles.dotsContainer}>
                                {Array.from({length: Math.min(day.eventCount, 4)}, (_, i) => <View key={i} style={styles.eventDot} />)}
                            </View>
                        }
                    </TouchableOpacity>
                ))}
